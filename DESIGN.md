...
SPA。ベースは FocusFlow(タスク管理)。**上部タブ**(`FFX.switchTab`・`ffx-tabbtn`) **タスク / すべて** でタスク管理面を切り替える。**下部タブバーは常設**(大きめ・`--tab-h: 80px`)で **お部屋 / プレゼント / おでかけ / きろく / せってい**。上部タブと同じトーン(テキスト+アクティブに線)で見せ、お部屋を実際に開いているときだけ「お部屋」を選択表示する(タスク/すべて中は下部は無印)。お部屋は下部「お部屋」から開く(`openRoom()` → `FFX.switchTab('room')`)。`refreshBottomNav()` は `App.switchTab`・`openRoom`・`FFX.switchTab` から呼ばれ `FFX.getCurrentTab()` を見てアクティブを決める。設計方針: タスク=情報優先・装飾控えめ、お部屋=没入優先・装飾可。お部屋のセリフ吹き出しは立ち絵のすぐ下に中央表示・やわらかいピンク枠(`#f2b3c8`)・本文16px。

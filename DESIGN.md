...
- **ステップの並べ替え**: 編集画面(`#ffx-screen-edit`)の「ステップ」見出し右の
  「並べ替え」ボタン(`#stepReorderBtn`、ステップが 2 つ未満なら非表示)。上のタスクの
  手番と同じ「タップした順に番号を振る」方式で、`toggleStepReorder()` /
  `stepReorderTap()` / `finalizeStepReorder()`。`order` フィールドは持たず、
  **`t.steps` 配列そのものを並べ替える**(配列順 = 実行順のため)
  - 並べ替え中はカードのタップがステップ詳細ではなく採番になり、「›」を隠す。
    ハイライトは同じく不透明色(`.step-list-card.reorder-tapped`)
  - 並べ替え中はスワイプ削除を無効化する(削除すると index がずれて採番が壊れるため)。
    同じ理由で「＋ ステップを追加」を押した時は先に確定する
  - 編集画面を閉じる時も確定する(`resetStepReorder(true)`)。開く時は持ち越さない
...
- **画面**: `#triage-overlay`(不透明度低めの黒背景+中央カード)。
  ...
  - **今日** → `FFX.triageTaskToday(id)` で優先度を `must` に、`dueDate` も今日にする。
    これで今日はもう期限切れ扱いにならず、終わらなければ翌日また棚卸しに出てくる

...
| `js/home.js` | ホーム画面(セリフポップアップ・タスクランチャー)・きろく | (App 層) |
...
SPA。`<section>` 切り替え方式。ナビは下部タブバー(**ホーム / ショップ / おでかけ / きろく / せってい** の 5 つ。タスクは下部タブから外し、ホームの大きめランチャーから開く)。
...
2. **ホーム**: 部屋背景(時間帯 morning/day/evening/night で空のグラデが変化、窓・棚・机・床の CSS 装飾)+ステータスバー(レベル・コイン・ストリーク)+立ち絵(クリックで `idle`。画像立ち絵に表情差分を登録していれば `homeIdleExpression()` がその差分を順に見せる)+**セリフポップアップ**(`#home-bubble`。部屋の下・タスクランチャーのすぐ上に出て立ち絵には被らない。長め(約 8 秒)で消え、消えるときに表情は休憩中の顔 `_homeRestExpr` に戻る)+**大きめのタスクランチャー**(`#home-task-launcher`。残タスク件数を表示し、タップでタスク画面へ)。タスク一覧そのものはホームに載せない
3. **タスク**: FocusFlow 移植版(上記「FocusFlow 統合」参照)。ホームのランチャーから開く。完了時はコイン・親密度付与+キャラのリアクション
...
- **ホーム**: タスク一覧は載せず、ランチャー(`#home-task-launcher`)に未完了タスク件数(非 scheduled)だけを `refreshHomeLauncher()` で表示。タップでタスク画面へ遷移する
...
- ホーム背景は CSS だけで描く部屋(窓+時間帯で変わる空のグラデ、棚・机・床のシルエット)。セリフはポップアップ(部屋の下・ランチャーのすぐ上に出て立ち絵に被らず、約 8 秒で消え、表情も休憩中に戻す)、ランチャー・ステータスバーはニュートラルなカードで載せる。デート VN のシーン背景も演出として鮮やかな配色を残す

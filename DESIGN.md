...
SPA。ベースは FocusFlow(タスク管理)。**上部タブ**(`FFX.switchTab`) **タスク / すべて** でタスク管理面を切り替える。**下部ナビは画面で姿を変える**(`refreshBottomNav()`):

- **タスク/すべて表示中**: コンパクト = お部屋へ行く 1 ボタンだけ(`.tab-bar.compact`、ピンクのピル)
- **お部屋・ゲーム各画面**: フルの下部タブバー **お部屋 / プレゼント / おでかけ / きろく / せってい**

お部屋は上部タブから外し、下部の「お部屋」ボタンで開く(`#ffx-tab-room` 自体は FocusFlow 埋め込み内のコンテンツのまま)。お部屋からタスクへ戻るときは上部の タスク/すべて タブを使う。`refreshBottomNav()` は `App.switchTab`・`openRoom`・`FFX.switchTab` から呼ばれ、`FFX.getCurrentTab()` で上部タブの状態を見てコンパクト/フルを決める。設計方針: タスク=情報優先・装飾控えめ、お部屋=没入優先・装飾可。

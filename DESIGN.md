...
| `js/home.js` | ホーム画面(常設セリフ欄・タスクランチャー)・きろく | (App 層) |
...
2. **ホーム**: 部屋背景(時間帯 morning/day/evening/night で空のグラデが変化、窓・棚・机・床の CSS 装飾)+ステータスバー(レベル・コイン・ストリーク)+立ち絵(クリックで `idle`。画像立ち絵に表情差分を登録していれば `homeIdleExpression()` がその差分を順に見せる)+**常設のセリフ欄**(`#home-bubble`。自動で消えず常に最新セリフを表示)+**大きめのタスクランチャー**(`#home-task-launcher`。残タスク件数を表示し、タップでタスク画面へ)。タスク一覧そのものはホームに載せない
...
- ホーム背景は CSS だけで描く部屋(窓+時間帯で変わる空のグラデ、棚・机・床のシルエット)。チャット周りのカード(セリフ欄・ランチャー・ステータスバー)はニュートラルで載せる。デート VN のシーン背景も演出として鮮やかな配色を残す

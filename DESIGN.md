...
2. **ホーム**: 部屋背景(時間帯 morning/day/evening/night で空のグラデが変化、窓・棚・机・床の CSS 装飾)+ステータスバー(レベル・コイン・ストリーク)+立ち絵(クリックで `idle`。画像立ち絵に表情差分を登録していれば `homeIdleExpression()` がその差分を順に見せる)+**セリフポップアップ**(`#home-bubble`。立ち絵の上にふわっと出て、長め(約 8 秒)で消える。消えるときに表情は休憩中の顔 `_homeRestExpr` に戻る)+**大きめのタスクランチャー**(`#home-task-launcher`。残タスク件数を表示し、タップでタスク画面へ)。タスク一覧そのものはホームに載せない
...
- ホーム背景は CSS だけで描く部屋(窓+時間帯で変わる空のグラデ、棚・机・床のシルエット)。セリフはポップアップ(立ち絵の上に出て約 8 秒で消え、表情も休憩中に戻す)、ランチャー・ステータスバーはニュートラルなカードで載せる。デート VN のシーン背景も演出として鮮やかな配色を残す

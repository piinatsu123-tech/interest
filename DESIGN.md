...
| `js/home.js` | お部屋(部屋背景・ステータスバー・立ち絵・セリフポップアップ・ハブ)・きろく | (App 層) |
...
SPA。ベースは FocusFlow(タスク管理)。上部タブバー **ホーム / すべて / お部屋** の 3 つで切り替え(`FFX.switchTab`)。ゲーム/キャラ要素は「お部屋」に集約し、そこから **プレゼント / おでかけ / きろく / せってい** をハブメニューで開く(下部タブバーは廃止)。設計方針: ホーム=情報優先・装飾控えめ、お部屋=没入優先・装飾可。
...
2. **ホーム(タスクダッシュボード)**: FocusFlow のメイン(タイムバー+緊急度グループ)。通常起動はここに着地。`information_first`・装飾控えめ。`＋追加` でタスク作成
3. **すべて**: タスク編集(期日タイムライン+全タスク一覧、緊急度の付け替え・削除)
4. **お部屋**(`#ffx-tab-room`): いっしょぐらしの生活面。部屋背景(時間帯 morning/day/evening/night で空のグラデが変化、窓・棚・机・床の CSS 装飾)+ステータスバー(レベル・コイン・ストリーク)+立ち絵(クリックで `idle`。画像立ち絵に表情差分を登録していれば `homeIdleExpression()` がその差分を順に見せる)+**セリフポップアップ**(`#home-bubble`。部屋の下に出て立ち絵には被らない。約 8 秒で消え、消えるとき表情は休憩中の顔 `_homeRestExpr` に戻る)+**ハブメニュー**(`.room-hub`。プレゼント/おでかけ/きろく/せっていへの入口)。タブに入ると `App.enterRoom()` が背景更新・スロットが変わっていれば挨拶。`＋追加` は隠す
   ...
   - 各サブ画面はヘッダーの `‹` もどるボタン(`App.openRoom()`)でお部屋に戻る
...
- お部屋背景は CSS だけで描く部屋(窓+時間帯で変わる空のグラデ、棚・机・床のシルエット)。セリフはポップアップ(部屋の下・ハブのすぐ上に出て立ち絵に被らず、約 8 秒で消え、表情も休憩中に戻す)、ハブ・ステータスバーはニュートラルなカードで載せる。お部屋は没入優先なので絵文字アイコンも許容。デート VN のシーン背景も演出として鮮やかな配色を残す

...
SPA。ベースは FocusFlow(タスク管理)。**上部タブ**(`FFX.switchTab`・同一デザインの `ffx-tabbtn`) **タスク / すべて / お部屋** の 3 つで切り替える。**下部ナビは画面で出し入れする**(`refreshBottomNav()`):

- **タスク/すべて表示中**: 下部バーを隠し(`.tab-bar.hidden`)、画面を最下部まで使う(`#tab-tasks.fullheight`)。お部屋へは上部「お部屋」タブで行く
- **お部屋・ゲーム各画面**: 下部タブバー **お部屋 / プレゼント / おでかけ / きろく / せってい** を表示(大きめ。`--tab-h: 80px`)

`refreshBottomNav()` は `App.switchTab`・`openRoom`・`FFX.switchTab` から呼ばれ、`FFX.getCurrentTab()` で上部タブの状態を見て下部バーの表示/非表示とアクティブを決める。設計方針: タスク=情報優先・装飾控えめ、お部屋=没入優先・装飾可。お部屋のステータスリボン(親密度/コイン/ストリーク)とセリフ吹き出し(立ち絵のすぐ下に中央表示・18px)、下部タブは大きめに見せる。

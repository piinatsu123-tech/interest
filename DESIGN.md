...
SPA。ベースは FocusFlow(タスク管理)。**上部タブ**(`FFX.switchTab`) **タスク / すべて / お部屋** の 3 つでタスク管理面とお部屋を切り替え、**下部タブバー**(`App.switchTab`/`openRoom`) **お部屋 / プレゼント / おでかけ / きろく / せってい** でお部屋とゲーム要素を行き来する。設計方針: タスク=情報優先・装飾控えめ、お部屋=没入優先・装飾可。下部タブの「お部屋」は `#tab-tasks` セクションを表示するため、上部タブが タスク/すべて のときも下部は「お部屋」がアクティブ表示になる。

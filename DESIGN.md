...
2. **ホーム**: 時間帯(morning/day/evening/night)でほんのり色づく淡い背景+ステータスバー(レベル・コイン・ストリーク)+立ち絵(クリックで `idle`)+**常設のセリフ欄**(`#home-bubble`。自動で消えず常に最新セリフを表示)+**大きめのタスクランチャー**(`#home-task-launcher`。残タスク件数を表示し、タップでタスク画面へ)。タスク一覧そのものはホームに載せない
...
- ホーム背景は時間帯で淡く色づくだけのグラデ(家具の装飾は廃止)。デート VN のシーン背景だけは演出として鮮やかな配色を残す
